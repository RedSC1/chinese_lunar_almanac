...
import '../models/sanyuan_jiuyun.dart';
 ...
 */
  ...
  static const List<({CompassDirection direction, int baseStarNumber})>
  luoshuGrid = [
  ...
  static FlyingStar getDayStar(
    ...
    final idx = useHistoricalSolarTerms
        ? _historicalMode(timePack, isConsecutive)
        : _preciseDayMode(timePack, isConsecutive, exactJieQiTime);
    return FlyingStar.values[idx];
  }
  ...
  /// * [false] (0点对齐): 保证全天12个时辰的遁局统一。只要夏至落于今日，今日0点起即统一切换。
  /// * [true] (分秒): 严格按交节时刻切换，交接当天节气交界点会从阳遁顺飞直接跳跃到阴遁逆飞的绝对坐标）。
  static FlyingStar getHourStar(
    TimePack timePack, {
    DayFlyingStarMethod method = DayFlyingStarMethod.consecutive,
    bool useHistoricalSolarTerms = false,
    bool exactJieQiTime = false,
  }) {
    final int idx = _getHourStarIdx(
      timePack,
      method: method,
  ...
  static int _getHourStarIdx(
    ...
    final int hourIdx = ((timePack.virtualTime.hour + 1) ~/ 2) % 12;
    return ((fixedIdx + hourIdx * direction) % 9 + 9) % 9;
  }

  static bool getDunYinYang(
    TimePack timePack, {
    DayFlyingStarMethod method = DayFlyingStarMethod.consecutive,
    bool useHistoricalSolarTerms = false,
    bool exactJieQiTime = false,
    ...
    bool direction = (!useHistoricalSolarTerms)
        ? _preciseDay(timePack, enableOffset, exactJieQiTime).$2
        : _historicalDay(timePack, enableOffset).$2;
    return direction;
  }
  ...
  static (int jzDay, bool direction) _preciseDay(
    TimePack timePack,
    bool enableOffset,
    bool exactJieQiTime,
  ) {
  ...
  static int _historicalMode(TimePack timePack, bool enableOffset) {
    int jzDay;
    bool direction = true;
  ...
  static (int jzDay, bool direction) _historicalDay(
    TimePack timePack,
    bool enableOffset,
  ) {
    final ssq = SSQ();
    // 历史模式仍用北京时间比较
    final SSQResult tb = ssq.calcY(timePack.bjClt.toJ2000());
...
enum DayFlyingStarMethod { consecutive, discontinuous }
...
enum Mountain {
  ...
  static bool getFlyDirectionByStarNumber(
    int starNumber,
    YuanLong targetDragon,
  ) {
